...
#
#                LOOKUP STRATEGY:
#                  Serial number -> Intune managedDevices (filtered by $TargetOS, bulk pull)
...
#
# Change Log   :
#   v1.0 - 2026-04-15 - Sethu Kumar B - Initial release.
#   v1.1 - 2026-04-15 - Sethu Kumar B - Added Windows OS filter.
#   v1.2 - 2026-04-15 - Sethu Kumar B - Made OS platform configurable via
#                        $TargetOS in config block. Supports Windows, macOS,
#                        iOS, Android, Linux, ChromeOS, or blank for all.
#                        URI filter built dynamically at runtime.
# ==============================================================================
...
# -- OS PLATFORM FILTER --------------------------------------------------------
# Controls which OS platform is pulled from Intune.
# Set to the platform you want to query. Leave blank to pull ALL platforms.
#
# Valid values  : Windows | macOS | iOS | Android | Linux | ChromeOS
# Leave blank   : $TargetOS = ""   <- pulls all platforms (slower for large fleets)
#
# Examples:
#   $TargetOS = "Windows"   <- Windows laptops and desktops only
#   $TargetOS = "macOS"     <- Mac devices only
#   $TargetOS = "iOS"       <- iPhones and iPads only
#   $TargetOS = "Android"   <- Android devices only
#   $TargetOS = ""          <- all platforms
# -----------------------------------------------------------------------------
$TargetOS = "Windows"
...
Write-Log "OS platform  : $(if ($TargetOS) { $TargetOS } else { 'All platforms' })" -Level INFO
Write-Log "" -Level BLANK
...
Write-Log "==========================================================" -Level SECTION
Write-Log "STEP 3 - Building Intune Device Lookup Table ($(if ($TargetOS) { $TargetOS } else { "All Platforms" }))" -Level INFO
Write-Log "==========================================================" -Level SECTION

# Build Intune URI - apply OS filter if $TargetOS is set, otherwise pull all platforms
if (-not [string]::IsNullOrWhiteSpace($TargetOS)) {
    $EncodedFilter = [Uri]::EscapeDataString("operatingSystem eq '$TargetOS'")
    $IntuneUri     = "https://graph.microsoft.com/beta/deviceManagement/managedDevices" +
                     "?`$filter=$EncodedFilter&`$top=1000"
    Write-Log "OS filter applied: $TargetOS" -Level INFO
} else {
    $IntuneUri = "https://graph.microsoft.com/beta/deviceManagement/managedDevices?`$top=1000"
    Write-Log "OS filter: ALL platforms (no filter applied)" -Level INFO
}
$AllIntune  = Invoke-GraphGetAllPages -InitialUri $IntuneUri -AccessToken $Token -Label "Intune managed devices ($(if ($TargetOS) { $TargetOS } else { 'All' }))"
