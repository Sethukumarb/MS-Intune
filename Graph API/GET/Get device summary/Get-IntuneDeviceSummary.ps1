...
# ------------------------------------------------------------------
# INPUT FILE
# Reads Hostnames.txt from the same folder as this script.
# Leave as-is unless you need to point to a different file name.
# ------------------------------------------------------------------
...
# ------------------------------------------------------------------
# OUTPUT FOLDER
# Saves CSV output to the same folder as this script.
# ------------------------------------------------------------------
...
Write-Host "   Intune Device Summary  |  Western Digital             " -ForegroundColor Cyan
...
if (-not [string]::IsNullOrWhiteSpace($InputFile) -and (Test-Path $InputFile -PathType Leaf)) {
    $mode = "file"
    Write-Log "Input file detected: $InputFile" -Level INFO
}
else {
    Write-Host ""
