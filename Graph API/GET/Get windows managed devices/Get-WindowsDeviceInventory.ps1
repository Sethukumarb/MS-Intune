...
#            Returns "N/A" for null/empty and "Never" for the Graph API
#            "0001-01-01T00:00:00Z" sentinel value.
# ─────────────────────────────────────────────────────────────────────────────
function ConvertTo-FriendlyDate {
...
#            Requires a separate per-device entity GET call — not available
#            via $select on the collection endpoint.
# ─────────────────────────────────────────────────────────────────────────────
function Get-LastLoggedOnUser {
    param (
...
# Purpose  : Queries /beta/deviceManagement/managedDevices with a Windows OS
#            filter and retrieves ALL available device fields in a single
#            paginated call. $select intentionally omitted — combining
#            $filter + $select causes HTTP 400 on this endpoint.
# ─────────────────────────────────────────────────────────────────────────────
function Get-AllWindowsDevices {
    param (
        [Parameter(Mandatory)][string]$AccessToken,
...
# Purpose  : Converts bytes to a rounded GB string. Returns "N/A" for null/zero.
# ─────────────────────────────────────────────────────────────────────────────
function Convert-StorageToGB {
    param ($Bytes)
...
$LogHeader = @"
================================================================================
  Windows Device Full Inventory  |  Sethu Kumar B
...
Write-Log "   Windows Device Full Inventory  |  Sethu Kumar B             " -Level SECTION
Write-Log "================================================================" -Level SECTION
