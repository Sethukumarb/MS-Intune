...
function Write-Log {
    ...
    # Log file output — silently skip if log file not yet initialised
    if ($script:LogFile) {
        try {
        ...
        catch { }   # Never block the script on a log write failure
    }
}
...
function Invoke-GraphGetAllPages {
    ...
    do {
        ...
        try {
            ...
            Write-Log ("  Page {0,3}  |  {1,5} records this page  |  Running total: {2}" -f `
...
function Get-WindowsDevices {
    ...
    # v1.0 endpoint — stable, no beta dependency required for default columns
...
function Shape-DeviceRecord {
    ...
    # Extract build number from full OS version string for OSBuildNumber column
    $BuildNumber = "N/A"
    if ($Device.osVersion -match "10\.0\.(\d+)") {
        $BuildNumber = $Matches[1]
    }

    # Calculate days since last sync for at-a-glance staleness check
    $DaysSinceSync = "N/A"
...
# Both CSV and log file are saved to $PSScriptRoot (script folder).
# $PSScriptRoot is set automatically by PowerShell when running a saved .ps1 file.
...
# Write log file header — establishes the file and records run metadata
$LogHeader = @"
================================================================================
  Windows Device Inventory (Simple)  |  Endpoint Engineering  |  Sethu Kumar B
...
try {
    ...
    $script:LogFile = $null   # Disable log — script continues without it
}

# ── Step 2: Console + log banner ──────────────────────────────────────────────
Write-Log "" -Level BLANK
Write-Log "================================================================" -Level SECTION
Write-Log "  Windows Device Inventory (Simple)  |  Endpoint Engineering   " -Level SECTION
Write-Log "================================================================" -Level SECTION
...
Write-Log "[INFO] Friendly OS     : Enabled (build number → release name)" -Level INFO
Write-Log "" -Level BLANK
