...
# ==============================================================================
# Script Name  : Get-IntuneAuditReport-AppAssignments.ps1
# Description  : Retrieves all Application changes from the Intune audit log
#                for a specified date range.
#
#                Captures the full lifecycle of application events:
...
#                  IntuneAudit_AppAssignments_[StartDate]_[EndDate]_[ts].csv
#                  IntuneAudit_AppAssignments_[StartDate]_[EndDate]_[ts].log
#
...
#       DeviceManagementApps.Read.All  — read application audit events
#   - PowerShell 5.1 or later
#
# Change Log   :
#   v1.0 - 2026-04-10 - Sethu Kumar B - Initial release.
#   v1.1 - 2026-04-10 - Sethu Kumar B - Fixed actor identity resolution with
...
#                        Full app lifecycle: Create, Delete, Assign,
...
#              Create          → APP ADDED TO INTUNE
#              Delete          → APP REMOVED FROM INTUNE
#              Assign          → APP ASSIGNED TO GROUP
#              RemoveReference → APP REMOVED FROM GROUP
#              Patch           → APP MODIFIED
...
function Get-ActionLabel {
    ...
    switch ($OperationType) {
        "Create"          { return "APP ADDED TO INTUNE" }
        "Delete"          { return "APP REMOVED FROM INTUNE" }
        "Assign"          { return "APP ASSIGNED TO GROUP" }
        "RemoveReference" { return "APP REMOVED FROM GROUP" }
        "Patch"           { return "APP MODIFIED" }
...
function ConvertTo-AuditRow {
    ...
    $Actor          = $Event.actor
    $Resolved       = Resolve-ActorIdentity -Actor $Actor
    $ChangedBy_UPN  = $Resolved.Identity
    ...
    # For app events the first resource is the app itself.
    # displayName = app name, auditResourceType = app type (e.g. Win32LobApp, MicrosoftStoreForBusinessApp)
    $Resource = if ($Event.resources -and $Event.resources.Count -gt 0) { $Event.resources[0] } else { $null }
    $AppName  = if ($Resource -and -not [string]::IsNullOrWhiteSpace($Resource.displayName)) {
                    $Resource.displayName
                } else { $Event.displayName }
    $AppType  = if ($Resource -and $Resource.auditResourceType) {
...
$OutputFile     = Join-Path $OutputFolder "IntuneAudit_AppAssignments_${DateLabel}_${Timestamp}.csv"
$script:LogFile = Join-Path $OutputFolder "IntuneAudit_AppAssignments_${DateLabel}_${Timestamp}.log"
...
try {
    [System.IO.File]::WriteAllText($script:LogFile,
        "Get-IntuneAuditReport-AppAssignments`r`nStarted: $(Get-Date)`r`nDate Range: $StartDate to $EndDate`r`n`r`n",
...
Write-Host "  Intune Audit — Application Changes & Assignments              " -ForegroundColor Cyan
...
Write-Log "Category     : Application"                                 -Level INFO
...
Write-Log "==========================================================" -Level SECTION
Write-Log "Server-side filter : category = Application"                    -Level INFO
...
# Server-side filter on category + date range.
# Graph API does not reliably support multiple activityOperationType values
# in a single $filter — operation type filtering done client-side below.
$FilterStr = "category eq 'Application'" +
...
$RawEvents = Invoke-GraphGetAllPages -InitialUri $AuditUri -AccessToken $Token `
             -Label "Application audit events ($StartDate to $EndDate)"

Write-Log "" -Level BLANK
Write-Log "Total Application events in date range: $($RawEvents.Count)" -Level INFO
...
if ($FilteredEvents.Count -eq 0) {
    ...
    Write-Log "Verify the date range and that application changes occurred." -Level WARN
    Write-Log "" -Level BLANK
}
...
$UniqueApps    = @($Results | Where-Object { $_.AppName -ne "N/A" } |
                   Select-Object -ExpandProperty AppName -Unique)
...
Write-Log "  APP ADDED TO INTUNE        : $CreatedCount"                     -Level $(if ($CreatedCount  -gt 0) {"SUCCESS"} else {"INFO"})
Write-Log "  APP REMOVED FROM INTUNE    : $DeletedCount"                     -Level $(if ($DeletedCount  -gt 0) {"WARN"}    else {"INFO"})
Write-Log "  APP ASSIGNED TO GROUP      : $AssignedCount"                    -Level $(if ($AssignedCount -gt 0) {"SUCCESS"} else {"INFO"})
Write-Log "  APP REMOVED FROM GROUP     : $RemovedCount"                     -Level $(if ($RemovedCount  -gt 0) {"WARN"}    else {"INFO"})
Write-Log "  APP MODIFIED               : $ModifiedCount"                    -Level $(if ($ModifiedCount -gt 0) {"WARN"}    else {"INFO"})
Write-Log "  ASSIGNMENT REL. ADDED      : $SetRefCount"                      -Level $(if ($SetRefCount   -gt 0) {"SUCCESS"} else {"INFO"})
Write-Log "" -Level BLANK
Write-Log "  ── Apps Affected ────────────────────────────────────────" -Level SECTION
Write-Log "  Unique apps affected       : $($UniqueApps.Count)"              -Level INFO
